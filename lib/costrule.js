...
let costRule = function costRuleFunc(inTimeRule,
  ...
  that.totalCost = function totalCostFunc(inDateSpans, inTZ=null) {
    ...
    // sort and merge the input timespans
    ...
    if(inDateSpans.length > 0) {
      ...
      const ruleSpans = timerule.generateDateSpans(earliestSpan.getBegin(),
                                                    latestSpan.getEnd());
      // identify all overlaps between costrule and input date-spans
      overlaps = caltime.intersectDateSpans(ruleSpans, inDateSpans);
      // decide how the number of time units should be measured
      ...
      measureResult = caltime.measureDateSpans(overlaps,
                                              durationType,
                                              inTZ);
    }
    // calculate the cost of each overlapping date-span
    switch(rateType) {
      ...
      case modconst.RATETYPE_PER_SECOND_ROUNDUP:
        const durationSecs = Math.floor(measureResult / 1000);
      ...
      case modconst.RATETYPE_PER_MINUTE_ROUNDUP:
        const durationMins = Math.floor(measureResult / caltime.constants.MSECS_PER_MIN);
      ...
      case modconst.RATETYPE_PER_HOUR_ROUNDUP:
        const durationHours = Math.floor(measureResult / caltime.constants.MSECS_PER_HOUR);
      ...
      case modconst.RATETYPE_PER_DAY_PRORATA:
        retval.cost = (measureResult / (that.getWorkDayDuration() * caltime.constants.MSECS_PER_MIN)) * rate;
        break;
      case modconst.RATETYPE_PER_DAY_ROUNDUP:
        const workDays = measureResult / (that.getWorkDayDuration() * caltime.constants.MSECS_PER_MIN);
        const wholeDays = Math.floor(workDays);
        retval.cost = wholeDays * rate;
        if(wholeDays < workDays) {
          retval.cost += rate;
        }
        break;
    ...
    // create array of date-spans used to calculate cost while
    // maintaining the order of the elements.
    for(i=0; i<overlaps.length; i++) {
      retval.usedSpans.push(overlaps[i]);
    }
    // calculate the remainder date-spans which did not contribute to cost
    i = 0;
